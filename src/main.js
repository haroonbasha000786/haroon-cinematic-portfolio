...
import { photoClip } from './lib/portrait.js';
import { heroWalkClip } from './lib/hero-walk.js';
...
async function main() {
  ...
  // This change is intentionally limited to the first section.
  app.clips.hero = await heroWalkClip();
  ...
  initUniverse().then((u) => { app.universe = u; })
    .catch((e) => console.warn('[haroon] universe unavailable:', e.message));
  initChrono().then((c) => { app.chrono = c; })
    .catch((e) => console.warn('[haroon] chrono unavailable:', e.message));
  initGallery().then((g) => { app.gallery = g; })
    .catch((e) => console.warn('[haroon] gallery unavailable:', e.message));
  initFinale().then((f) => { app.finale = f; })
    .catch((e) => console.warn('[haroon] finale unavailable:', e.message));
...
function degrade(reason) {
  console.warn('[haroon] falling back:', reason);
  Promise.allSettled([initUniverse(), initChrono(), initGallery(), initFinale()]);
  root.classList.remove('is-booting');
  root.classList.add('is-fallback');
  boot.classList.add('is-done');
  for (const [, name] of CUES) root.classList.add(`is-${name}`);
  document.querySelector('.stage-wrap').insertAdjacentHTML('afterbegin',
    '<div class="fallback"><p>HAROON</p>'
...
if (heroWrap && 'IntersectionObserver' in window) {
  ...
  if (spacer) {
    new IntersectionObserver(([e]) => {
      ...
      if (e.isIntersecting) { if (!reduced) c.play(); heroWrap.style.visibility = ''; }
...
document.addEventListener('visibilitychange', () => {
  ...
  for (const c of Object.values(app.clips)) {
    if (!c) continue;
    if (hidden) c.pause();
    else if (!reduced && app.heroOnScreen !== false) c.play();
  }
});
