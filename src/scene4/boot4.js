...
export async function initGallery() {
  ...
  if (profile.portrait) person.src = profile.portrait;
  ...
  const cards = CARDS.map((c, i) => {
    ...
    el.setAttribute('aria-label', `Project — ${projects[i].title}`);
    ...
    el.innerHTML = `<span class="g-card__in">`
      ...
      + `draggable="false" loading="eager" decoding="async"><span class="g-card__caption">${projects[i].title}</span></span>`;
    deck.appendChild(el);
    return el;
  });
